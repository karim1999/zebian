...
class OrderNow extends Component {
    ...
    componentDidMount() {
        ...
        ref.on('value', snapshot => {
            this.setState({
                cities: _.map(snapshot.val(), (value, key) => {
                    return {...value, key};
                })
            });
        })
            ...
            <AppTemplate navigation={nav} name="اطلب الان">
                <View style={{flex: 1, flexDirection: 'row', justifyContent: 'center'}}>
                    <View style={{width: '90%', flexDirection: 'column'}}>
                        ...
                        <ListCard header={'حدد مكان الاستلام'}
                                  footer={(this.props.order.recieveAddress != '') ? this.props.order.recieveAddress : 'اختر موقع استلام الشحنه'}
                                  rightIcon={MapLocation} onPress={() => {
                            nav.navigate('RecievePlace')
                        }} rightIconWidth={40}/>
                        <ListCard header={'حدد مكان التسليم'} onPress={() => {
                            nav.navigate('GivePlace')
                        }}
                                  footer={(this.props.order.giveAddress != '') ? this.props.order.giveAddress : 'اختر موقع تسليم الشحنه'}
                                  rightIcon={Navigation} rightIconWidth={40}/>
                        <ListCard onPress={() => {
                            nav.navigate('Time')
                        }} header={'حدد وقت التسليم'}
                                  footer={(this.props.order.time != '') ? this.props.order.time : 'اختر وقت تسليم الشحنه'}
                                  rightIcon={Clock} rightIconWidth={40}/>
                        <ListCard onPress={() => {
                            nav.navigate('CarType')
                        }} header={'حدد نوع السياره'}
                                  footer={(this.props.order.car != '') ? (this.props.order.car == 'car') ? 'سيدان ' : 'بيك اب' : 'بيك أب - سيدان'}
                                  rightIcon={Car} rightIconWidth={40}/>
                        <Card picker>
                            <Picker
                                mode="dropdown"
                    ...
                    <Form style={{marginHorizontal: 14}}>
                        <View style={{width: '80%', alignSelf: 'center'}}>
                            ...
                            <Text style={{
                                ...
                                fontSize: 17,
                                fontFamily: 'Droid Arabic Kufi'
                            }} note>اكتب هنا تفاصيل الغرض الذي ترغب في ارساله مثلا ما هو وكيف حجمه</Text>
                        </View>
                        <Textarea onChangeText={
                            (text) => {
                                this.setState({desc: text})
                            }
                        } value={this.state.desc} style={{
                            borderRadius: 14,
                            backgroundColor: 'white',
                            textAlign: 'center',
                            fontFamily: 'Droid Arabic Kufi'
                        }} rowSpan={4} bordered placeholder=""/>
                    </Form>
                </View>
                <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center'}}>
                    <Button onPress={() => this.OrderNow(nav)} rounded style={{
